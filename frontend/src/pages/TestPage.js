...
import { QuestionBody, OptionBody } from '../components/QuestionMedia';
...
const getSubjectDisplayName = (subject) => {
  ...
  return mapping[subject?.toLowerCase()?.trim()] || subject;
};
...
const [timeLeft, setTimeLeft] = useState(0);
  ...
  const handleSubmitTest = useCallback(async () => {
    ...
    try {
      ...
      navigate(`/result/${response.data.id}`, { replace: true });
    } catch (error) {
      console.error(error);
      setSubmitting(false);
    }
  ...
  if (!examStarted) {
    return (
      <div className="min-h-screen bg-[#0f172a] text-white flex items-center justify-center p-4">
        <div className="bg-white/10 backdrop-blur-xl border border-white/10 rounded-3xl p-8 w-full max-w-2xl shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h2 className="text-3xl font-black mb-2 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">
              Exam Instructions
            </h2>
            <h3 className="text-xl font-bold text-white mb-8">{test.title}</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
                <p className="text-gray-400 text-sm mb-1">Subject</p>
                <p className="text-lg font-bold text-white capitalize">{getSubjectDisplayName(test.subject) || "General"}</p>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
                ...
                <p className="text-lg font-bold text-white">{test.total_duration_minutes} Minutes</p>
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4">
                <p className="text-gray-400 text-sm mb-1">Marking Scheme</p>
                <p className="text-lg font-bold">
                  <span className="text-green-400">+{test.marks_per_correct} Correct</span>
                  {test.negative_marking_enabled ? (
                    <span className="text-red-400 ml-3">-{test.negative_marks} Incorrect</span>
                  ) : (
                    <span className="text-gray-400 ml-3">No Negative</span>
                  )}
                </p>
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-blue-500 to-cyan-400 text-white font-bold text-lg hover:scale-[1.02] transition-transform shadow-lg shadow-blue-500/25"
            >
              Start Exam
            </button>
          </div>
      ...
      <div className="xl:col-span-3">
        ...
        <div className="bg-white/5 border border-white/10 backdrop-blur-xl rounded-3xl p-4 md:p-6 shadow-2xl">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 mb-6">
            ...
            {subjects.map((subject) => (
              <button
                ...
                className={`px-5 py-2 rounded-2xl text-sm font-semibold transition-all whitespace-nowrap ${
          ...
          <div className="mb-8">
            ...
            {/* OPTIONS */}
            {(() => {
              const isImageMode = question?.use_image_display && question?.question_image_url;
              const hasNoOptionText = !question.option_a && !question.option_b && !question.option_c && !question.option_d;

              if (isImageMode && hasNoOptionText) {
                return (
                  <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-3xl p-5 md:p-6 mb-6 text-center shadow-lg">
                      ...
                      {["A", "B", "C", "D"].map((option) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-14 h-14 md:w-16 md:h-16 rounded-2xl border-2 font-bold text-xl md:text-2xl transition-all duration-300 flex items-center justify-center ${
                              isSelected
                                ? "border-blue-500 bg-gradient-to-br from-blue-600 to-cyan-500 text-white shadow-lg shadow-blue-500/30 scale-105"
                                : "border-white/10 bg-white/5 text-gray-300 hover:bg-white/10 hover:border-white/20 hover:scale-[1.02]"
                            }`}
                          >
                            {option}
                          </button>
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div className="text-base md:text-lg">
                            ...
                            <OptionBody question={question} option={option} optionText={optionText} />
                          </div>
                        </div>
                      </div>
                    );
